...
export async function validateUploadedFile(
  ...
  if (file.size > options.maxSize) {
    logger.warn(
      `${options.context}: oversized upload rejected (${file.size} bytes)`,
    );
  ...
  const imageAllowed =
    options.allowImages &&
    detectedMimeType &&
    allowedImageMimeTypes.has(detectedMimeType);
  const videoAllowed =
    options.allowVideos &&
    detectedMimeType &&
    allowedVideoMimeTypes.has(detectedMimeType);

  if (!detectedMimeType || (!imageAllowed && !videoAllowed)) {
    logger.warn(
      `${options.context}: unsupported upload rejected (${declaredMimeType || 'unknown'})`,
    );
  ...
  if (detectedMimeType.startsWith('image/')) {
    try {
      ...
      if (
        ...
        logger.warn(
          `${options.context}: image dimensions rejected (${metadata.width}x${metadata.height})`,
        );
