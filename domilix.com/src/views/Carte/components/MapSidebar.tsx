...
export default function MapSidebar({
  ...
  return (
    <aside className="hidden md:flex h-full w-[376px] min-w-[376px] p-2 pr-0">
      ...
      <div className="flex gap-1 overflow-x-auto border-b border-gray-100 px-2 scrollbar-none">
        {TABS.map((tab) => (
          <button
            key={tab.key}
            type="button"
            onClick={() => onTabChange(tab.key)}
            className={`relative shrink-0 whitespace-nowrap px-3 py-2.5 text-xs font-semibold transition ${
              activeTab === tab.key
                ? 'text-brand-500'
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-3 py-2 text-xs text-gray-400">
          ...
          {activeTab === 'unlocked' && `${unlockedListings.length} annonce${unlockedListings.length > 1 ? 's' : ''} débloquée${unlockedListings.length > 1 ? 's' : ''}`}
        </div>
        ...
        {activeTab === 'unlocked' && (
          <div className="px-3 pb-4 space-y-2">
            {unlockedListings.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-8">Aucune annonce débloquée</p>
            ) : (
